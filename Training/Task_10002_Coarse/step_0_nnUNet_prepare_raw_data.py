import pickle

import SimpleITK as sitk
import os
import numpy as np
import json
import math

import scipy.ndimage
import skimage.morphology

import task_setting
import config

from Utils.utils_simpleitk import *
from Utils.bbox import *


def get_dataset():
    image_dir = config.data_path["image"]
    gt_dir = config.data_path["gt_artery"]
    heart_dir = config.data_path["gt_heart"]
    data_split = json.load(open("../../data_split.json", 'r'))

    save_dir = f'{task_setting.path_nnunet_raw_data}/{task_setting.task_name}'
    save_dir_image = f"{save_dir}/imagesTr"
    save_dir_label = f"{save_dir}/labelsTr"
    os.makedirs(save_dir_image, exist_ok=True)
    os.makedirs(save_dir_label, exist_ok=True)

    new_spacing = config.spacing["Task_10002_Coarse"]  # (Z, Y, X)

    list_train_case_ids = data_split["train"]
    print(f"==> Total {len(list_train_case_ids)} train cases")

    count = 0
    for case_id in list_train_case_ids:

        image_nii = sitk.ReadImage(f"{image_dir}/{case_id}.nii.gz")
        gt_nii = sitk.ReadImage(f"{gt_dir}/{case_id}.nii.gz")
        heart_nii = sitk.ReadImage(f"{heart_dir}/{case_id}.nii.gz")

        # To LPS
        image_nii = to_orientation(image_nii)
        gt_nii = to_orientation(gt_nii)
        heart_nii = to_orientation(heart_nii)

        # Check geometry
        same_geometry = compare_geometry_multiple([image_nii, gt_nii, heart_nii])
        if not same_geometry:
            raise Exception(f"{case_id} has not same geometry !")

        count += 1
        print(f"==> {count}: {case_id}")

        # Make gt
        gt = sitk.GetArrayFromImage(gt_nii)
        aorta = gt == 1
        vessel = gt == 2

        aorta_nii = sitk.GetImageFromArray(np.uint8(aorta))
        vessel_nii = sitk.GetImageFromArray(np.uint8(vessel))
        aorta_nii = copy_nii_info(image_nii, aorta_nii)
        vessel_nii = copy_nii_info(image_nii, vessel_nii)

        # Resampling
        image_nii = resample(image_nii, new_spacing, interp=sitk.sitkLinear)
        heart_nii = resample_to_template(heart_nii, template_nii=image_nii, dtype=sitk.sitkUInt8, constant_value=0)
        aorta_nii = resample(
            aorta_nii,
            new_spacing=image_nii.GetSpacing(),
            new_origin=image_nii.GetOrigin(),
            new_size=image_nii.GetSize(),
            new_direction=image_nii.GetDirection(),
            interp=sitk.sitkLinear,
            dtype=sitk.sitkFloat32,
            constant_value=0,
            UseNearestNeighborExtrapolator=False
        )
        vessel_nii = resample(
            vessel_nii,
            new_spacing=image_nii.GetSpacing(),
            new_origin=image_nii.GetOrigin(),
            new_size=image_nii.GetSize(),
            new_direction=image_nii.GetDirection(),
            interp=sitk.sitkLinear,
            dtype=sitk.sitkFloat32,
            constant_value=0,
            UseNearestNeighborExtrapolator=False
        )

        aorta = sitk.GetArrayFromImage(aorta_nii)
        vessel = sitk.GetArrayFromImage(vessel_nii)

        gt = np.uint8(aorta >= 0.5)
        gt[vessel >= 0.1] = 2
        gt_nii = sitk.GetImageFromArray(gt)
        gt_nii = copy_nii_info(image_nii, gt_nii)

        # Crop ROP
        heart = sitk.GetArrayFromImage(heart_nii)
        bbox = get_bbox(heart > 0)
        bz, ez, by, ey, bx, ex = bbox
        image_nii= image_nii[bx:ex+1, by:ey+1, bz:ez+1]
        gt_nii= gt_nii[bx:ex+1, by:ey+1, bz:ez+1]

        # Get Centerline
        gt = sitk.GetArrayFromImage(gt_nii)
        cl = skimage.morphology.skeletonize(gt == 2)
        cl = skimage.morphology.binary_dilation(cl)

        cl[gt != 2] = 0
        cl_nii = sitk.GetImageFromArray(np.uint8(cl))
        cl_nii = copy_nii_info(image_nii, cl_nii)

        # Saving
        sitk.WriteImage(image_nii, f"{save_dir_image}/{case_id}_0000.nii.gz")
        sitk.WriteImage(cl_nii, f"{save_dir_image}/{case_id}_0001.nii.gz")
        sitk.WriteImage(gt_nii, f"{save_dir_label}/{case_id}.nii.gz")

        print(f"        ----> Done ")


def get_dataset_info():
    save_dir = f"{task_setting.path_nnunet_raw_data}/{task_setting.task_name}"
    save_dir_label = f"{task_setting.path_nnunet_raw_data}/{task_setting.task_name}/labelsTr"

    # Train case ids
    numTraining = 0
    for file in os.listdir(save_dir_label):
        if file.find('.nii.gz') > -1:
            numTraining += 1
    print(f"==> Total {numTraining} train cases")

    # Dataset json
    dataset_info = {
        "channel_names": {  # must belong to ['CT', 'noNorm', 'zscore', 'rescale_to_0_1', 'rgb_to_0_1']
            "0": "CT",
            "1": "noNorm",
        },
        "labels": {
            "background": 0,
            "1": 1,
            "2": 2,
        },
        "numTraining": numTraining,
        "file_ending": ".nii.gz",
        # "regions_class_order": [1, 2],
    }

    with open(os.path.join(save_dir, "dataset.json"), 'w') as f:
        json.dump(dataset_info, f, indent=4, sort_keys=False)


def main():
    get_dataset()
    get_dataset_info()


if __name__ == '__main__':
    main()
