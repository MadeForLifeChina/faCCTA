...
from transforms import CustomTransform, CustomTransformVal
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        if os.environ.get('nnUNet_sample_foreground_p') is not None:
            self.oversample_foreground_percent = float(os.environ.get('nnUNet_sample_foreground_p'))
        else:
        ...
        self.num_epochs = int(os.environ.get('nnUNet_max_num_epochs'))
    ...
    def get_plain_dataloaders(self, initial_patch_size: Tuple[int, ...], dim: int):
        ...
        if dim == 2:
            ...
            ##############################################
            dl_tr = CustomDataLoader(dataset_tr, self.batch_size,
                                       initial_patch_size,
                                       self.configuration_manager.patch_size,
                                       self.label_manager,
                                       oversample_foreground_percent=self.oversample_foreground_percent,
                                       sampling_probabilities=None, pad_sides=None)
            dl_val = CustomDataLoader(dataset_val, self.batch_size,
                                        self.configuration_manager.patch_size,
                                        self.configuration_manager.patch_size,
                                        self.label_manager,
                                        oversample_foreground_percent=self.oversample_foreground_percent,
    ...
    def get_training_transforms(patch_size: Union[np.ndarray, Tuple[int]],
        ...
        tr_transforms = CustomTransform(tr_transforms)
    ...
    def get_validation_transforms(deep_supervision_scales: Union[List, Tuple],
        ...
        val_transforms = CustomTransformVal(val_transforms)
    ...
    def on_train_epoch_start(self):
        ...
        #################################################################
        self.print_to_log_file("max epoch: ", self.num_epochs)
        self.print_to_log_file("GPU ID: ", os.environ['CUDA_VISIBLE_DEVICES'])
        self.print_to_log_file("fold: ", self.fold)
        self.print_to_log_file("output dir: ", self.output_folder)
        self.print_to_log_file("optimizer: ", str(type(self.optimizer)))
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            #############################################################
            use_time_this_epoch = time() - time_start
            may_need_time_to_finish = (self.num_epochs - self.current_epoch - 1) * use_time_this_epoch / 3600.
            self.print_to_log_file(
                "This epoch took %f s, may still need %f hours \n" % (use_time_this_epoch, may_need_time_to_finish))
