...
class DatasetFingerprintExtractor(object):
    ...
    def collect_foreground_intensities(segmentation: np.ndarray, images: np.ndarray, seed: int = 1234,
        ...
        for i in range(len(images)):
            ...
            ########################################################
            if task_setting.clip_range is not None:
                p_005 = task_setting.clip_range[str(i)]['percentile_00_5']
                p_995 = task_setting.clip_range[str(i)]['percentile_99_5']
                foreground_pixels = np.clip(foreground_pixels, a_min=p_005, a_max=p_995)
    ...
    def run(self, overwrite_existing: bool = False) -> dict:
        ...
        if not isfile(properties_file) or overwrite_existing:
            ...
            for i in range(num_channels):
                ...
                ########################################################
                if task_setting.clip_range is not None:
                    if 'percentile_00_5' in task_setting.clip_range[str(i)].keys():
                        percentile_00_5 = task_setting.clip_range[str(i)]['percentile_00_5']
                    else:
                        percentile_00_5 = float(np.percentile(foreground_intensities_per_channel[i], 0.5))

                    if 'percentile_99_5' in task_setting.clip_range[str(i)].keys():
                        percentile_99_5 = task_setting.clip_range[str(i)]['percentile_99_5']
                    else:
                        percentile_99_5 = float(np.percentile(foreground_intensities_per_channel[i], 99.5))

                    if 'mean' in task_setting.clip_range[str(i)].keys():
                        mean_v = task_setting.clip_range[str(i)]['mean']
                    else:
                        mean_v = float(np.mean(foreground_intensities_per_channel[i]))

                    if 'std' in task_setting.clip_range[str(i)].keys():
                        std_v = task_setting.clip_range[str(i)]['std']
                    else:
                        std_v = float(np.std(foreground_intensities_per_channel[i]))
                else:
                    percentile_00_5 = float(np.percentile(foreground_intensities_per_channel[i], 0.5))
                    percentile_99_5 = float(np.percentile(foreground_intensities_per_channel[i], 99.5))
                    mean_v = float(np.mean(foreground_intensities_per_channel[i]))
